...
export default function WomenSafety() {
  ...
  return <CitizenLayout title="Women Safety"><div className="mx-auto max-w-6xl space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-black text-ink">Private safety support</h2><p className="text-sm text-slate-500">Your reports, identity, and exact location are restricted to you, Emergency Command, and assigned responders.</p></div><span className={`rounded-full px-3 py-1 text-xs font-black ${online ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'}`}>{online ? (live ? 'Live updates connected' : 'Online') : 'Offline'}</span></div>
    ...
    <section className="rounded-2xl border border-purple-200 bg-purple-50/70 p-5 shadow-sm"><div className="flex flex-wrap items-center justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.15em] text-purple-700">Discreet emergency</p><h3 className="mt-1 text-xl font-black text-ink">I'm Not Safe</h3><p className="mt-1 max-w-xl text-sm text-slate-600">Use this when you are being harassed, stalked, followed, threatened, or need a discreet security response. Hold the button for three seconds.</p></div><SOSButton onActivate={createDiscreetEmergency} captureLocation={captureLocation} label={'I\'M NOT SAFE\nHOLD'} title="HOLD FOR DISCREET HELP" busy={busy} /></div><p className="mt-3 text-xs text-purple-800">If GPS is unavailable, use the detailed safety report below and enter the nearest safe address or landmark. CivicSync will not pretend a location was captured.</p></section>
    <div className="grid gap-5 lg:grid-cols-[1.15fr_.85fr]"><section className="civic-card p-5"><h3 className="text-lg font-black text-ink">Report a safety concern</h3><p className="mt-1 text-sm text-slate-500">For harassment, stalking, suspicious behavior, unsafe areas, or unsafe transport. Emergency Command decides the final response.</p><form onSubmit={submit} className="mt-5 space-y-4">
      ...
      <fieldset className="space-y-3 rounded-xl border border-slate-200 p-3"><legend className="px-1 text-sm font-black text-ink">Location</legend><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={captureLocation} className="civic-secondary">{geolocation.loading ? 'Locating…' : '📍 Capture my location'}</button>{coords && <span className="text-xs font-bold text-emerald-700">✓ Location captured</span>}{geolocation.error && <span className="text-xs font-bold text-amber-700">{geolocation.error}</span>}</div><input value={form.address} onChange={(event) => setForm({ ...form, address: event.target.value })} maxLength={300} placeholder="Safe address or nearest landmark" aria-label="Safe address or landmark" /><EmergencyMap height="h-56" showControls={false} autoFit={Boolean(coords)} onMapClick={setCoords} emergencies={coords ? [{ _id: 'women-safety-pin', emergencyId: 'Location', title: 'Selected location', category: 'women_safety', severity: 'high', status: 'reported', createdAt: new Date().toISOString(), location: coords }] : []} /><p className="text-xs text-slate-500">Tap the map to choose a location manually. Sensitive locations are never placed on the public safety map.</p></fieldset>
      <button disabled={busy} className="civic-primary">{busy ? 'Sending…' : 'Send restricted safety report'}</button>
    </form></section>
    <section className="civic-card p-5"><h3 className="text-lg font-black text-ink">My safety incidents</h3><p className="mt-1 text-sm text-slate-500">Only your own restricted reports are shown here.</p><div className="mt-4 space-y-3">{loading ? <LoadingState text="Loading your safety reports…" /> : incidents.length ? incidents.map((incident) => <EmergencyCard key={incident._id} emergency={incident} onOpen={(item) => navigate(`/dashboard/citizen/emergency/${item._id}`)} />) : <p className="rounded-xl border border-dashed border-slate-300 p-6 text-center text-sm text-slate-500">No women-safety reports yet.</p>}</div></section>
    </div>

    <section className="civic-card p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h3 className="text-lg font-black text-ink">Safety map and services</h3><p className="text-sm text-slate-500">Public map shows services and aggregate areas only — never your identity or exact report location.</p></div><button onClick={() => navigate('/dashboard/citizen/nearby-services')} className="civic-secondary">Open nearby services</button></div><div className="mt-3"><EmergencyMap height="h-64" emergencies={[]} facilities={facilities} responders={[]} showControls={false} /></div></section>
    <section className="civic-card p-5"><h3 className="text-lg font-black text-ink">Nearby support</h3><p className="text-sm text-slate-500">Hospitals, police stations, safe points, and other registered services.</p><div className="mt-4 grid gap-2 sm:grid-cols-2">{facilities.slice(0, 6).map((facility) => <NearbyServiceCard key={facility._id} facility={facility} />)}</div></section>
  </div></CitizenLayout>;
}
