  ...
  return (
    <CitizenLayout title={`Tracking ${emergency.emergencyId}`}>
      <div className="mx-auto max-w-4xl space-y-5">
        ...
        {routeState.created && <p className="rounded-lg bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">Emergency submitted. Emergency Command has been notified.</p>}
        ...
        <section className="civic-card p-5">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <p className="text-sm font-black text-red-700">{emergency.emergencyId}</p>
                ...
                {emergency.discreet && <span className="rounded-full bg-slate-200 px-2 py-0.5 text-[10px] font-black uppercase text-slate-600">Discreet</span>}
              </div>
              <h2 className="mt-1 text-2xl font-black text-ink">{emergency.title}</h2>
              <p className="text-sm text-slate-500">{label(emergency.category)}{emergency.subcategory && emergency.subcategory !== 'other' ? ` · ${emergency.subcategory.replaceAll('_', ' ')}` : ''} · reported {new Date(emergency.createdAt).toLocaleString()}</p>
            </div>
          </div>
          <p className="mt-3 text-sm leading-6 text-slate-700">{emergency.description || 'No additional description provided.'}</p>
          <p className="mt-2 text-sm text-slate-600">📍 {emergency.location?.address || 'Location shared privately with responders'}</p>
          ...
          {emergency.status === 'requires_backup' && <p className="mt-3 rounded-lg bg-red-50 p-3 text-sm font-bold text-red-700">⚠ Backup has been requested for this incident — command is organizing additional teams.</p>}
        </section>

        <section className="civic-card p-5">
          <h3 className="font-black text-ink">Response timeline</h3>
          <div className="mt-3"><EmergencyTimeline emergency={emergency} /></div>
        </section>

        <section className="civic-card p-5">
          <h3 className="font-black text-ink">Response teams</h3>
          {emergency.responseAssignments?.length ? (
            <div className="mt-3 space-y-2">
              {emergency.responseAssignments.map((assignment) => (
                <article key={assignment._id} className="rounded-lg border border-slate-200 p-3 text-sm">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <p className="font-bold text-ink">{label(assignment.responseType)} {assignment.responseTeam ? `· ${assignment.responseTeam}` : ''}</p>
                    <EmergencyStatusBadge value={assignment.status} />
                  </div>
                  <p className="text-slate-600">Officer: {assignment.emergencyOfficer?.name || 'Unassigned'} · {assignment.fieldWorkers?.length || 0} field worker(s)</p>
                  {assignment.etaMinutes != null && <p className="text-xs text-slate-500">ETA: {assignment.etaMinutes} min</p>}
                </article>
              ))}
            </div>

          ) : <p className="mt-2 text-sm text-slate-500">No response team dispatched yet. Emergency Command is reviewing the incident.</p>}
        </section>
        ...
        {Number.isFinite(emergency.location?.latitude) && (
          <section className="civic-card p-5">
            <h3 className="font-black text-ink">Incident map</h3>
            <div className="mt-3">
              <EmergencyMap
                height="h-72"
                showControls={false}
        ...
        {isActive && emergency.visibility !== 'restricted' && (
          <section className="civic-card p-5">
            <h3 className="font-black text-ink">Community verification</h3>
            <p className="text-sm text-slate-500">Confirm what you can see. Verification counts are shared — no private details are exposed.</p>
            ...
            <p className="mt-2 text-xs text-slate-500">Still happening: {verification.stillHappening || 0} · Resolved: {verification.resolvedVotes || 0}</p>
            {verifyNotice && <p className="mt-2 rounded bg-emerald-50 p-2 text-sm font-semibold text-emerald-700">{verifyNotice}</p>}
          </section>
        )}
      </div>
    </CitizenLayout>
