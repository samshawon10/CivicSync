...
function DepartmentShell({ children, active, setActive }) {
  ...
  const sidebar = <aside className="flex h-full flex-col bg-ink p-5 text-white"><div><Link to={isHead ? '/dashboard/department-head' : '/dashboard/department-officer'} className="text-xl font-extrabold">Civic<span className="text-civic-300">Sync</span></Link><p className="mt-2 text-xs font-semibold uppercase tracking-wider text-slate-400">{user.departmentName || 'Department workspace'}</p><div className="mt-6 rounded-xl border border-white/10 bg-white/5 p-3"><p className="text-sm font-bold">{user.name}</p><p className="mt-1 text-xs text-slate-300">{isHead ? 'Department Head' : 'Department Officer'}</p></div></div><nav className="mt-6 space-y-1">{items.map(([key, label, icon]) => <button key={key} onClick={() => go(key)} className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-semibold transition ${active === key ? 'bg-white text-ink shadow-sm' : 'text-slate-200 hover:bg-white/10'}`}><span className="grid h-7 w-7 place-items-center rounded-md bg-white/10 text-[10px] font-black">{icon}</span>{label}</button>)}</nav><button onClick={signOut} className="mt-auto rounded-lg border border-white/15 px-3 py-2 text-sm font-bold text-white hover:bg-white/10">Logout</button></aside>;
...
function Complaints({ isHead }) {
  ...
  async function load() {
    ...
    try {
      const [{ data }, staffResult] = await Promise.all([departmentApi.reports(filters), isHead ? departmentApi.staff() : Promise.resolve({ data: { staff: [] } })]);
