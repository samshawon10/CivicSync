...
export default function EmergencyLayout({ title, children }) {
  ...
  return <div className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[250px_1fr]">
    ...
    <div className="min-w-0"><header className="flex min-h-16 items-center border-b border-slate-200 bg-white px-4 sm:px-8"><div><p className="text-xs font-bold tracking-[.14em] text-civic-600">EMERGENCY OPERATIONS</p><h1 className="text-lg font-black text-ink">{title}</h1></div></header><main className="p-4 sm:p-7">{children}</main></div>
  </div>;
}
