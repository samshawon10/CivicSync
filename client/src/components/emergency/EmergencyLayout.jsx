...
export default function EmergencyLayout({ title, children }) {
  ...
  return <div className="min-h-screen bg-slate-50 lg:grid lg:grid-cols-[250px_1fr]">
    ...
    <div className="min-w-0"><header className="flex min-h-16 items-center border-b border-slate-200 bg-white px-4 sm:px-8 dark:border-slate-800 dark:bg-slate-900"><div><p className="text-xs font-bold tracking-[.14em] text-civic-600">EMERGENCY OPERATIONS</p><h1 className="text-lg font-black text-ink dark:text-white">{title}</h1></div><button type="button" onClick={() => search.open()} className="ml-auto grid h-10 w-10 place-items-center rounded-lg text-slate-600 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-slate-800" aria-label="Search CivicSync"><Search size={18} /></button><button type="button" onClick={() => ai?.openCopilot()} className="ml-2 inline-flex min-h-10 items-center gap-1.5 rounded-lg border border-civic-500/40 bg-civic-500/10 px-3 text-sm font-bold text-civic-700 transition hover:bg-civic-500/20 dark:text-civic-300" aria-label="Open CivicSync Intelligence"><Sparkles size={16} /><span className="hidden sm:inline">AI</span></button></header><main className="p-4 sm:p-7">{children}</main></div>
  </div>;
}
